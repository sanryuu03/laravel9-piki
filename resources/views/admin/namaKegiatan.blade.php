  ...
  .<div class="container-fluid landingpage-anggota">
        <div class="row">
          <div class="col-md-4">
              <a href="{{ route('backend.form.add.nama.kegiatan') }}" class="btn btn-warning btn-sm mb-1">Tambah Nama Kegiatan</a>
          </div>
      </div>
      <div class="card-body">
          <table id="table_id" class="table table-bordered table-striped table-anggota">
              ...
              <tbody>
                  @foreach($namaKegiatan as $item)
                  <tr>
                      <td>{{ $loop->iteration }}</td>
                      <td>{{ $item->posAnggaran->nama_pos_anggaran }}</td>
