          ...
          <div class="container-fluid">
              <form method="post" action="{{ route('sponsorShipSatu.store') }}" enctype="multipart/form-data">
                  ...
                  <div class="form-group">
                    ...
                    <input type="hidden" name="id" class="form-control" value="{{ $sponsorShipSatu->id ?? '' }}">
                    @if($sponsorShipSatu->picture_path ?? '')
                    <div></div><img width="150px" src="{{ url('/storage/assets/sponsorshipsatu/'.$sponsorShipSatu->picture_path) }}"></div>
                  @endif
                  <input type="file" name="picture_path" class="form-control">
          </div>
          <div class="form-group">
              <label>{{ ucwords('link web') }}</label>
              <input type="text" name="link_web" class="form-control @error('link_web') is-invalid @enderror" value="{{ old('link_web',$sponsorShipSatu->link_web ?? '') }}">
              @error('link_web')
